...
首页、列表、系列页、项目页、RSS 和 Sitemap 从文章集合自动更新。
...
首页重点文章取 `featured: true` 且已发布的最新内容；列表按 `publishedAt` 排序；系列按 `seriesOrder` 排序。
...
不需要制作文章列表卡片，不需要改首页，不需要复制页眉和页脚，也不需要单独维护 SEO 文件。
